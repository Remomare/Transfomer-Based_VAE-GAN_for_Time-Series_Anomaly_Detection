...
from utils import  vae_loss, set_random_seed, get_tb_experiment_name
...
def main(args):
    ...
    if args.vae_setting==True:
        loss_fn = vae_loss(pad_idx=0, anneal_function=args.kl_anneal_function, x0=int(len(dataloaders)*args.epoch/2)).to(device)
    else:
    ...
    for epoch_idx in range(0, args.epoch):
        train_epoch(args, epoch_idx, model, dataloaders, optimizer, scheduler, loss_fn, writer, device)
